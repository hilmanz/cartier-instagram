			...
			if(@isset($this->apps->session->getSession('twitter_session','twitter_permission')->loginPermission)){
				...
				if ($code == 200) {
					...
					if(!$this->apps->session->get('user')){
						if(isset($thisMobile))	header("location:{$CONFIG['MOBILE_SITE']}service/content/checklogintwitters?pagetype=login");
						else header("location:{$CONFIG['BASE_DOMAIN']}service/content/checklogintwitters?pagetype=login");
						exit;
					}else{
						if(isset($thisMobile))	header("location:{$CONFIG['MOBILE_SITE']}service/content/checklogintwitters?pagetype=login");
						else header("location:{$CONFIG['BASE_DOMAIN']}service/content/checklogintwitters?pagetype=login");
